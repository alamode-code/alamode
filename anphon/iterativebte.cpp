...
Iterativebte::Iterativebte(PHON *phon) :
    Pointers(phon)
{
...
void Iterativebte::get_triplets()
{
    ...
    for (unsigned int i = 0; i < nklocal; ++i) {
        ...
        // k3 = k1 - k2
        dos->kmesh_dos->get_unique_triplet_k(ik,
                                             symmetry->SymmList,
...
void Iterativebte::calc_damping4()
{
    ...
    if (mympi->my_rank == 0) {
        ...
        conductivity->interpolate_data(conductivity->kmesh_4ph,
                                       dos->kmesh_dos,
                                       conductivity->damping4,
                                       damping4_dense);
...
void Iterativebte::calc_kappa(int itemp, double ***&df, double **&kappa_out)
{
    ...
    const double conversionfactor = Ryd / (time_ry * Bohr_in_Angstrom * 1.0e-10 * nk_3ph * system->get_primcell().
                                           volume);
