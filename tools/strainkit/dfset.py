...
def build_dfset(disp_results, ref_atoms, offset=None, dmag=None):
    ...
    if offset is not None:
        ...
        f_off = offset.forces
        if f_off is None:
            raise ValueError(f"{offset.path}: no forces found in the offset calculation")
        if np.abs(d_off).max() > 1.0e-4:
            warnings.warn(f"{offset.path}: the undisplaced structure moved by up to "
    ...
    if dmag is not None and len(u):
        ...
        if not (0.5 * dmag <= umax <= 2.0 * dmag):
            warnings.warn(f"largest displacement {umax:.3e} A is far from the expected "
    ...
    with open(path, "w") as fp:
        for k, (uu, ff) in enumerate(zip(u_ang, f_ev_ang)):
            ...
            for a in range(len(uu)):
                ...
                fp.write("{:15.7F} {:15.7F} {:15.7F} {:20.8E} {:15.8E} {:15.8E}\n".format(
