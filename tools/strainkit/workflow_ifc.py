...
from .fcsorder import (anphon_primitive_cell, check_supercell_equivalence, describe_ordering,
                       match_primitive_atoms, read_anphon_cell, read_fcs_structure,
...
from .strain import (StrainPoint, check_weight_sums, default_modes, deform,
                     deformation_gradient, ifc_strain_set, load_modes_json, modes_from_names)
from .structure import OUTPUT_FILENAME, normalize_code, read_template, write_structure
from .writers import anphon_file_locations, write_strain_force_in, write_strain_harmonic_in
...
def _points_from_manifest(manifest):
    return [StrainPoint(e["label"], e["mode"], float(e["smag"]), float(e["weight"]), np.array(e["u"]))
            for e in manifest["entries"]]


def generate(coupling, code, template_dir, outdir=".", structure_file=None, smag=None, dmag=0.01,
             central=False, no_offset=False, modes_json=None, mode_names=None, nbody=2, cutoff=None,
             job_template=None, dft_command=None, copy_potcar=False, force=False, log=print):
    ...
    existing = [os.path.join(outdir, _dirname(k)) for k in range(first, first + n_dirs)
                if os.path.exists(os.path.join(outdir, _dirname(k)))]
    if existing and not force:
        raise FileExistsError(f"{len(existing)} strain_* directories already exist in {outdir}; use --force to overwrite")
    for d in existing:
        shutil.rmtree(d)
    ...
    if coupling == "harmonic":
        ...
        for k, p in enumerate(points, start=1):
            ...
            write_structure(template, strained, os.path.join(d, "nodisp"), link_potcar=not copy_potcar)
            disp_dirs = []
            for m, s in enumerate(structs, start=1):
                sub = f"disp_{m:0{width}d}"
                write_structure(template, s, os.path.join(d, sub), link_potcar=not copy_potcar)
            ...
            entries.append({
                "index": k, "dir": dname, "label": p.label, "mode": p.mode, "smag": p.smag,
                "weight": p.weight, "u": p.u, "n_disp": len(structs), "disp_dirs": disp_dirs,
                "nodisp_dir": None if no_offset else "nodisp",
                "patterns": [[[int(i), np.asarray(v).tolist()] for i, v, _ in pat] for pat in patterns],
            })
            log(f"  {dname}: mode {p.mode:>3s} smag {p.smag:+.5f} weight {p.weight:g}  "
                f"-> {len(structs)} displacement patterns" + ("" if no_offset else " + nodisp"))
    else:
        # reference (unstrained) primitive cell: strain_000
        write_structure(template, template.atoms, os.path.join(outdir, _dirname(0), "primitive"),
        ...
        for k, p in enumerate(points, start=1):
            dname = _dirname(k)
            d = os.path.join(outdir, dname)
            write_structure(template, deform(template.atoms, p.F), os.path.join(d, "primitive"),
                            link_potcar=not copy_potcar)
            if job_text:
                with open(os.path.join(d, "job.sh"), "w") as f:
                    f.write(render_job_script(job_text, dft_text or "", ["primitive"]))
            all_calc_dirs.append(os.path.join(dname, "primitive"))
            entries.append({"index": k, "dir": dname, "label": p.label, "mode": p.mode,
                            "smag": p.smag, "weight": p.weight, "u": p.u, "primitive_dir": "primitive"})
            log(f"  {dname}: mode {p.mode:>3s} smag {p.smag:+.5f} weight {p.weight:g}")
    write_run_all(os.path.join(outdir, "run_all.sh"), all_calc_dirs, dft_text)

    manifest = {
        "tool": "strainifc", "code": code, "coupling": coupling,
        "template_dir": template.template_dir, "structure_file": template.structure_name,
        "output_file": OUTPUT_FILENAME[code], "dmag": float(dmag), "central": bool(central),
        "offset": not no_offset, "nbody": int(nbody), "cutoff": None if cutoff is None else float(cutoff),
    ...
    log("  NOTE: the atom order of the template is kept in every generated structure; anphon's "
...
def collect_harmonic(outdir, manifest, fcs=None, unchecked=False, fcs_format="xml", prefix="strain",
                     results_dir=RESULTS_DIR, last=False, solver="dense", write_dfset_files=False, log=print):
    ...
    if fcs:
        ...
        raise ValueError("give the reference force-constant file used by anphon (--fcs REF.xml|.h5) "
    ...
    for e in manifest["entries"]:
        ...
        offset = None
        if e.get("nodisp_dir"):
            offset = _read(os.path.join(d, e["nodisp_dir"], manifest["output_file"]), code, last)
            check_geometry(offset, strained)
        patterns = [[(i, np.array(v), "Cartesian") for i, v in pat] for pat in e["patterns"]]
        ...
        info = fit_harmonic(strained, u, f, out, fcs_format, manifest["nbody"], manifest["cutoff"], solver)
        if fcs_struct is not None:
            verify_generated_fcs(out, fcs_struct, F)
        rows.append((e["mode"], e["smag"], e["weight"], out_name))
        log(f"  {e['dir']}: {info['nsnap']} snapshots, {info['n_free_parameters']} free parameters -> {out_name}")
    ...
    try:
        ...
        log("  NOTE: not all strain components are covered; use RENORM_3TO2ND = 3 (symmetry completion)")
...
def _anphon_mapping(ref_atoms, fcs, anphon_cell, reorder, log):
    ...
    if fcs is None:
        if anphon_cell is not None:
            raise ValueError("--anphon-cell requires --fcs (anphon folds the FCS supercell into &cell)")
        log("  NOTE: no --fcs given; strain_force.in rows are written in the order of the template "
    ...
    if n == len(ref_atoms):
        if not np.array_equal(mapping, np.arange(n)):
            msg = ("the atom order of the DFT primitive cell differs from anphon's primitive-cell order "
...
def collect_force(outdir, manifest, fcs=None, anphon_cell=None, results_dir=RESULTS_DIR, last=False,
                  reorder=False, log=print):
    code = manifest["code"]
    ref_atoms = _reference_atoms(manifest, outdir)
    ...
    if os.path.exists(ref_out):
        ...
        if fmax > 1.0e-3:
            warnings.warn("the reference structure is not well relaxed (max |F| > 1e-3 eV/A); "
                          "the strain-force coupling then depends on the residual forces")
    else:
        log("  NOTE: no reference calculation output (strain_000) found; forces of the unstrained cell "
            "are assumed to vanish")
    blocks = []
    for e in manifest["entries"]:
        F = deformation_gradient(np.array(e["u"]))
        r = _read(os.path.join(outdir, e["dir"], e["primitive_dir"], manifest["output_file"]), code, last)
...
def collect(outdir=".", coupling=None, fcs=None, anphon_cell=None, unchecked=False, fcs_format="xml",
            prefix="strain", results_dir=RESULTS_DIR, last=False, reorder=False, solver="dense",
            write_dfset_files=False, log=print):
    outdir = os.path.abspath(outdir)
    manifest = load_manifest(os.path.join(outdir, IFC_MANIFEST), expect_tool="strainifc")
    coupling = coupling or manifest["coupling"]
    if coupling != manifest["coupling"]:
        raise ValueError(f"the manifest was generated for coupling='{manifest['coupling']}'")
    if coupling == "harmonic":
        return collect_harmonic(outdir, manifest, fcs, unchecked, fcs_format, prefix, results_dir,
                                last, solver, write_dfset_files, log)
    return collect_force(outdir, manifest, fcs, anphon_cell, results_dir, last, reorder, log)
...
def check(outdir=".", fcs=None, anphon_cell=None, log=print):
    ...
    ref_atoms = _reference_atoms(manifest, outdir)
    fcs_struct = read_fcs_structure(fcs)
    log(f"Reference force-constant file: {fcs} ({fcs_struct.fmt}, {fcs_struct.nat} atoms, "
