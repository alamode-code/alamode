...
def _write_template(tmp_path, atoms):
    ...
    d.mkdir()
    ase.io.write(
        d / "POSCAR", atoms, format="vasp", direct=True, sort=False, vasp5=True
    )
...
def test_grouped_template_round_trip(ase_mod, tmp_path):
    ...
    assert [os.path.basename(f) for f in t.extra_files] == [
        "INCAR",
        "KPOINTS",
        "POTCAR",
    ]
    ...
    assert (
        p.endswith("POSCAR")
        and os.path.islink(out / "POTCAR")
        and (out / "INCAR").exists()
    )
    back = ase_mod.io.read(p, format="vasp")
    assert list(back.numbers) == list(sc.numbers)
    assert np.allclose(back.cell[:], sc.cell[:])
    p2 = structure.write_structure(
        t, t.atoms, str(tmp_path / "out2"), link_potcar=False
    )
