...
def test_lattice_rows_round_trip(ase_mod, alm_mod, tmp_path):
    ...
    assert np.allclose(fcs.xf - cell.get_scaled_positions(), np.round(fcs.xf - cell.get_scaled_positions()), atol=1e-8)


@pytest.mark.skipif(not os.path.exists(ALM_BIN), reason="alm binary (_build/alm/alm) not found")
def test_python_matches_cli_on_skewed_cell(ase_mod, alm_mod, tmp_path):
    ...
    lines = ["&general", "  PREFIX = cli", "  MODE = optimize", "  NAT = %d" % len(cell), "  NKD = 1", "  KD = Cu", "/",
             "&interaction", "  NORDER = 1", "/", "&optimize", "  DFSET = DFSET", "/",
             "&cell", "  1.0"]
    ...
    with open(work / "cli.log", "w") as log:
        proc = subprocess.run([ALM_BIN, "cli.in"], cwd=work, stdout=log, stderr=subprocess.STDOUT)
    ...
    def fc2(path):
        with h5py.File(path, "r") as h:
            ...
            key = np.concatenate([g["atom_indices_supercell"][:].reshape(n, -1),
                                  g["coord_indices"][:].reshape(n, -1),
                                  np.round(g["shift_vectors"][:].reshape(n, -1) * 1000).astype(int)], axis=1)
