...
def _fd_constants(atoms, h=1e-4):
    ...
    u = np.zeros((3, 3)); u[0, 0] = h
    d = (stress(u) - stress(-u)) / (2 * h) * GPA
    c11, c12 = d[0, 0], d[1, 1]
    u = np.zeros((3, 3)); u[1, 2] = u[2, 1] = h / 2
...
def cu_workdir(tmp_path_factory, ase_mod, spglib_mod):
    ...
    work = root / "work"
    we.generate("ase", str(tmpl), str(work), smag=0.005, nmag=2, dirset="minimal", log=lambda *a: None)
...
def test_stress_fit_matches_finite_differences(cu_workdir):
    ...
    assert abs(c2[0, 0] - c11) < 1.0 and abs(c2[0, 1] - c12) < 1.0 and abs(c2[3, 3] - c44) < 1.0
    # cubic symmetry after symmetrization, and small symmetrization change
    assert np.allclose(c2[0, 0], c2[1, 1]) and np.allclose(c2[3, 3], c2[5, 5]) and abs(c2[0, 3]) < 1e-9
    assert summary["symmetrization_max_change_eV_per_A3"]["C2"] * GPA < 0.1
    assert fit.rank == 83
    # files
    c2f, c3f = read_elastic_constants_in(os.path.join(work, "results", "elastic_constants.in"))
    assert c2f.shape == (9, 9) and c3f.shape == (9, 9, 9)
    s0 = read_C1_array_in(os.path.join(work, "results", "C1_array.in"))
    assert np.abs(s0).max() * 13.605693122994 / summary["volume_dft_A3"] * GPA < 0.05  # relaxed: sigma0 ~ 0
...
def test_both_fit_and_show(cu_workdir):
    ...
    text = we.show(os.path.join(work, "results", "elastic_constants.in"),
                   volume_A3=summary["volume_dft_A3"], c1_path=os.path.join(work, "results", "C1_array.in"))
