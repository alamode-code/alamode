...
def test_full_contraction_matches_design_rows():
    ...
    e_full = (np.einsum("ij,ij", strain.sym_from_voigt(s0), eta)
              + 0.5 * np.einsum("ijkl,ij,kl", c4, eta, eta)
              + np.einsum("ijklmn,ij,kl,mn", c6, eta, eta, eta) / 6.0)
    e_row = ef.design_row_energy(strain.voigt_from_sym(eta)) @ np.concatenate([s0, c2, c3])
    assert abs(e_full - e_row) < 1e-14
    # pure shear and C44-only checks
    c2s = np.zeros(21); c2s[ef.POS2[(3, 3)]] = 1.0  # C_yz,yz
    ...
    e_row = ef.design_row_energy(strain.voigt_from_sym(eta_s)) @ np.concatenate([np.zeros(6), c2s, np.zeros(56)])
...
def test_cubic_symmetrization(ase_mod, spglib_mod):
    ...
    m = ef.voigt66(cs)
    assert np.allclose(m[0, 0], m[1, 1]) and np.allclose(m[0, 1], m[0, 2]) and np.allclose(m[3, 3], m[5, 5])
    ...
    tri = ase_mod.Atoms("CuAg", cell=[[3, 0.2, 0.1], [0.3, 3.5, 0.2], [0.1, 0.4, 4.0]],
